...
# YENİ: Model Seçimi (GARCH vs EGARCH)
...
# --- ANA ALGORİTMA ---
def run_quant_analysis(df_input, target_date_str, p_size, f_window, thresh, g_p, g_q, vol_type):
    ...
    # =======================================================
    # TAB 2: VOLATİLİTE (GARCH veya EGARCH)
    # =======================================================
    with tab2:
        try:
            ...
            if vol_type == "EGARCH":
                ...
                res = am.fit(disp='off', update_freq=0)
                # GARCH analitik olarak çözülebilir
            ...
            ax_garch.fill_between(future_dates, garch_lower_path, garch_upper_path, color='green', alpha=0.2, label=f"%95 Güven Aralığı ({vol_type})")
    ...
    # =======================================================
    # TAB 3: TIMESFM (Google AI)
    # =======================================================
    with tab3:
        tfm_context_len = min(512, target_idx) 
        
        if tfm_context_len < 32:
            st.warning("TimesFM'in çalışabilmesi için hedeften önce en az 32 mum geçmiş veri olmalıdır.")
        else:
            with st.spinner("TimesFM Derin Öğrenme Modeli Başlatılıyor..."):
                tfm_model, is_loaded = load_timesfm_model(horizon_len=f_window)
                
                if not is_loaded:
                    st.error(f"TimesFM kütüphanesi yüklenemedi.\nDetay: {tfm_model}")
                else:
                    try:
                        tfm_input_data = prices[target_idx - tfm_context_len + 1 : target_idx + 1]
                        
                        forecast_result = tfm_model.forecast([tfm_input_data], freq=[0])
                        
                        point_forecast = forecast_result[0][0]
                        quantiles = forecast_result[1][0] 
                        
                        tfm_mean_path = np.insert(point_forecast, 0, current_entry_price)
                        tfm_upper_path = np.insert(quantiles[:, 8], 0, current_entry_price) 
                        tfm_lower_path = np.insert(quantiles[:, 1], 0, current_entry_price) 
                        
                        c1, c2, c3, c4 = st.columns(4)
                        c1.metric("Giriş Fiyatı", f"{current_entry_price:.2f}")
                        c2.metric("Yapay Zeka Ort. Hedef", f"{tfm_mean_path[-1]:.2f}")
                        c3.metric("AI %90 İyimser Senaryo", f"{tfm_upper_path[-1]:.2f}")
                        c4.metric("AI %20 Kötümser Senaryo", f"{tfm_lower_path[-1]:.2f}")

                        fig_tfm, ax_tfm = plt.subplots(figsize=(12, 4))
                        
                        plot_past_dates = current_dates[-p_size:]
                        plot_past_prices = current_pattern[-p_size:]
                        
                        ax_tfm.plot(plot_past_dates, plot_past_prices, label="Geçmiş Fiyat", color='black', linewidth=2)
                        
                        ax_tfm.fill_between(future_dates, tfm_lower_path, tfm_upper_path, color='#1f77b4', alpha=0.2, label="AI Olasılık Bandı (%20 - %90)")
                        ax_tfm.plot(future_dates, tfm_mean_path, color='#1f77b4', linestyle='--', linewidth=2.5, label="TimesFM Nokta Tahmini")
                        
                        if actual_projected is not None:
                            ax_tfm.plot(future_dates[:len(actual_projected)], actual_projected, label="GERÇEKLEŞEN", color='red', linewidth=3)

                        ax_tfm.axvline(x=current_dates[-1], color='black', linestyle=':')
                        ax_tfm.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
                        ax_tfm.legend(loc='upper left', prop={'size': 9})
                        ax_tfm.grid(True, alpha=0.3)
                        fig_tfm.autofmt_xdate(rotation=45)
                        st.pyplot(fig_tfm)
...
if run_button:
    if df is not None:
        with st.spinner("Modeller Hesaplanıyor..."):
            # Analiz fonksiyonuna vol_model_type (GARCH/EGARCH) değişkeni de gönderiliyor
            run_quant_analysis(df, st.session_state.target_date_val, pattern_size, forward_window, threshold, garch_p, garch_q, vol_model_type)
    else:
        st.error("Lütfen önce sol menüden bir CSV dosyası yükleyin.")
else:
    st.info("Sol menüden CSV dosyanızı yükleyin ve 'Modellerin Tamamını Çalıştır' butonuna basın.")